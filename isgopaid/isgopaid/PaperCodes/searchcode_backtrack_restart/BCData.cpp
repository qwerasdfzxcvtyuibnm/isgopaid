...
tuple<vector<uint8_t>, vector<uint8_t>, vector<vector<uint8_t>>>
BCData::MiddleSearch(uint const indexOutput, uint const indexInput){
	...
	while(int(indexInput) >= 0){
		...
		*/
			...
			//Fix the input
			m.addConstr(m.getVarByName("x_0_"+to_string(indexInput)) == 0);
			...
			//The first solution in the solution pool is the best one
			//
			auto nbtrails = m.get(GRB_IntAttr_SolCount);
...
tuple<vector<uint8_t>, vector<uint8_t>, vector<vector<uint8_t>>>
BCData::MiddleSearch_backtrack(uint const indexOutput, uint const indexInput){
	...
	m.set(GRB_IntParam_SolutionLimit, rMiddle);
